            ...
            <div className="grid grid-cols-2 gap-3">
                {generations.map((gen, index) => (
                    <motion.div
                        ...
                        className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-white/5 border border-white/10 group cursor-pointer shadow-lg shadow-black/10"
                    >
                        {/* Type Badge */}
                        {gen.type === 'video' && (
                        ...
                        {gen.type === 'video' ? (
                            ...
                            <img src={gen.image_url} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity duration-500 group-hover:scale-105 transition-transform" />
                        )}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                            <p className="text-[11px] text-white/90 font-medium line-clamp-2 leading-snug">
                                {gen.prompt}
                            </p>
                            <div className="flex items-center gap-1.5 mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity delay-75">
                                <span className="text-[9px] bg-white/10 px-1.5 py-0.5 rounded text-white/70 backdrop-blur-sm border border-white/5">
                ...
                {selectedItem && (
                    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4">
                        ...
                        <motion.div
                            ...
                            className="relative bg-[#1c1c1e] w-full max-w-sm rounded-t-[2.5rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl border-t sm:border border-white/10 max-h-[90vh] flex flex-col"
                        >
                            {/* Drag Handle (Mobile) */}
                            <div className="absolute top-3 left-1/2 -translate-x-1/2 w-12 h-1.5 bg-white/20 rounded-full z-50 sm:hidden" />
                            ...
                            <div className="p-6 overflow-y-auto">
                                <div className="mb-6">
                                    ...
                                    <div className="flex gap-2 text-xs text-white/40 font-mono overflow-x-auto pb-2 scrollbar-none">
                                        <span className="bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl whitespace-nowrap">{selectedItem.model_id}</span>
                                        <span className="bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl whitespace-nowrap uppercase">{selectedItem.ratio || '1:1'}</span>
                                        <span className="bg-white/5 border border-white/5 px-3 py-1.5 rounded-xl whitespace-nowrap">{new Date(selectedItem.created_at).toLocaleDateString()}</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        ...
                                        className="col-span-2 py-4 bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-400 hover:to-violet-500 text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-indigo-500/25 relative overflow-hidden group"
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all border backdrop-blur-md ${selectedItem.is_public
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="py-3.5 bg-white/5 hover:bg-white/10 text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-all border border-white/5 backdrop-blur-md"
                                    >
                                        <Download size={18} />
                                        <span>Save</span>
                                    </a>
