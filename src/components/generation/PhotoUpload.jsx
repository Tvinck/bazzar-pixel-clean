        ...
        <AnimatePresence mode="wait">
            {maxImagesForModel > 0 && (
                <motion.div
                    ...
                    className="px-4"
                >
                    <div className="bg-bg-secondary/80 backdrop-blur-md border border-white/5 rounded-[18px] p-4 shadow-lg">
                        ...
                        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                            {previewUrls.map((url, idx) => (
                                <div key={idx} className="relative flex-shrink-0 group">
                                    <div className="w-24 h-24 rounded-input overflow-hidden border border-white/10 shadow-md">
                            ...
                            {selectedImages.length < maxImagesForModel && (
                                <button
                                    ...
                                    className="w-24 h-24 rounded-input bg-bg-elevated/60 border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-1.5 text-text-secondary hover:text-white hover:border-[#3390ec]/50 hover:bg-bg-elevated transition-all active:scale-95 group"
                                >
