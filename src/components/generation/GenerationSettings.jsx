    ...
    return (
        <motion.div variants={itemVariants} className="space-y-4">
            <div className="px-5 flex items-center gap-2">
                <Settings2 size={18} className="text-[#8e8e93]" />
            ...
            <div className="px-4 md:px-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                ...
                {currentModel?.inputs?.audio === 'boolean' && (
                    <div className="flex items-center justify-between bg-[#1c1c1e] rounded-[10px] px-4 h-11 col-span-2 md:col-span-1">
                        ...
                        <button
                            ...
                            className={`w-10 h-6 rounded-full transition-colors relative ${customValues.audio ? 'bg-[#34c759]' : 'bg-[#3a3a3c]'}`}
                        >
                ...
                {currentModel?.modes && (
                    <div className="relative col-span-2 md:col-span-1">
                        <label className="text-[13px] font-semibold text-[#8e8e93] mb-1.5 block ml-1">Режим (Mode)</label>
                        <button
                        ...
                        {openDropdown === 'mode' && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-[#2c2c2e] rounded-[10px] z-20 shadow-xl overflow-hidden">
                                {currentModel.modes.map(m => (
                                    <button key={m} onClick={() => { setCustomValues(p => ({ ...p, mode: m })); setOpenDropdown(null); }} className="w-full text-left px-4 py-3 text-[15px] text-white border-b border-white/5 last:border-0 hover:bg-[#3a3a3c] capitalize">
                                        {m.replace(/-/g, ' ')}
                                    </button>
                                ))}
                            </div>
